...
class TestPatternClassificationViews:
    ...
    def test_declassify_view(self, client: "Client", django_user_model: "User"):
...
class TestBackupViews:
    def test_backup_create_new_view(self, client: "Client", django_user_model: "User"):
    ...
    def test_backup_purge_view(self, client: "Client", django_user_model: "User"):
    ...
    def test_backup_download_view(self, client: "Client", django_user_model: "User"):
        # Setup
    ...
    def test_backup_restore_view(self, client: "Client", django_user_model: "User"):
        # Setup
        ...
        try:
            ...
            assert tr_model.objects.count() == 2
            assert (
                float(tr_model.objects.aggregate(models.Sum("amount"))["amount__sum"])
                == 9.53
            )
...
class TestCreateViews:
    ...
    def create_view_test(
        ...
        try:
            assert response.status_code == 302 and model_cls.objects.count() == 1
            if user_required:
    ...
    def test_pattern_create_view(self, client, django_user_model):
        ...
        obj_params = dict(pattern="TestObj", category=parents["budget.Category"].id)
    ...
    def test_upload_create_view(self, client, django_user_model):
        ...
        content = ",".join([acc.date_col_name, acc.amt_col_name, acc.desc_col_name])
